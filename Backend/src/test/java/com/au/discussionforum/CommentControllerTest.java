...
class CommentControllerTest {
	...
	Comment comment1= new Comment();
       ...
       ques2.setMarked(false);
		...
		User user3= new User(user2.getUserId(),user2.getEmail(),user2.getPassword(),user2.getUsername(),user2.getPhoto());
		
		@SuppressWarnings("unused")
		Topic topic3= new Topic(topic2.getTopicId(),topic2.getTopicName());
		
		@SuppressWarnings("unused")
		Question ques3= new Question(ques2.getQuesId(),ques2.getUser(),ques2.getTopic(),ques2.getTitle(),ques2.getBody(),ques2.isMarked());
		
		@SuppressWarnings("unused")
		Answer ans3= new Answer(ans2.getAnsId(),ans2.getUser(),ans2.getQuestion(),ans.getAnswerBody(),ans2.isCorrect());
	...
	void addCommentTest() throws JsonProcessingException, Exception {
		...
		// User user= userService.getUserByUserId(comm.getUserId());
		 User user= new User();
		 user.setUserId(1);
		...
		// Answer answer = answerService.getAnswerByAnswerId(comm.getAnsId());
		  ...
		  comment.setAnswer(ans);
			...
			//MvcResult mvcResult =
					mockMvc.perform(
											post(url)
//											.contentType("application/json")
//											.content(objectMapper.writeValueAsString(comment.getCommentBody()))
											).andExpect(status().isOk()).andReturn();
			
			//String actualJsonResponse = mvcResult.getResponse().getContentAsString();
//			String expectedJsonResponse = objectMapper.writeValueAsString(comment.getCommentBody());
//
//		     assertEquals(actualJsonResponse,expectedJsonResponse);
	}
